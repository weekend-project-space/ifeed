...
//        channelResults.replaceAll((strategyId, list) -> {
...
//                });
//            }
//            return list.stream().map(i -> i.withScore(adaptiveScoreMapper.map(i.score()))).collect(Collectors.toList());
//        });
        // 将各通道结果交给融合层进行重排、多样化
        ...
        return new RecallResponse(fused, channelResults, context, latency, Map.of());
    }
}
