...
public class DatabaseEmbeddingStore implements EmbeddingStore {
    ...
    public Optional<float[]> getItemVector(Long itemId) {
        ...
        Optional<UUID> maybeUuid = articleRepository.findUidByArticleId(itemId);
        if (maybeUuid.isEmpty()) {
            return Optional.empty();
        }

        List<float[]> vectors = articleEmbeddingRepository.findAllByIds(List.of(maybeUuid.get())).stream()
